...
describe('App definition', () => {
  test('exports the auth, hydrator, every trigger, every action and the search', () => {
    ...
    expect(Object.keys(App.triggers).sort()).toEqual(['form_list', 'public_link_submission_abandoned', 'public_link_submission_created', 'public_link_submission_updated', 'request_canceled', 'request_completed', 'request_expired', 'request_list'])
    expect(App.triggers.form_list.display.hidden).toBe(true)
    expect(App.triggers.request_list.display.hidden).toBe(true)
